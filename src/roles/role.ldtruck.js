const roleLDTruck = {
    ...
    pickupEnergy(creep) {
        if (creep.memory.sourceRoom !== creep.room.name) {
            creep.travelTo(new RoomPosition(25, 25, creep.memory.sourceRoom), { visualizePathStyle: { stroke: '#ffffff' }, reusePath: 50 });
            return;
        }
        const droppedEnergy = creep.pos.findClosestByRange(FIND_DROPPED_RESOURCES)
        if (droppedEnergy) {
            if (creep.pickup(droppedEnergy) === ERR_NOT_IN_RANGE) {
                creep.travelTo(droppedEnergy, { visualizePathStyle: { stroke: '#ffffff' }, reusePath: 50 });
            }
        } else {
            // console.log('Error: Dropped energy not found for', creep.name);
            creep.say('No energy');
        }
    },
